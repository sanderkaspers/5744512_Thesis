"""
This script combines, analyzes, and visualizes mutation testing results from the 'cot' experiment.
...
# PATHS
cot_result_path = "results/cot"
yaml_folder_cot = os.path.join(cot_result_path, 'filtered_mutation_results')
csv_file_cot = os.path.join(cot_result_path, 'classification_summary_cot.csv')
cot_plots_path = os.path.join(cot_result_path, 'plots')
metrics_table_csv = os.path.join('results', 'cot', 'appendix_metrics_table_cot.csv')
coverage_json = os.path.join('results', 'cot', 'coverage_per_test_cot.json')
...
# LOAD classification_summary_cot.csv as df_classification
print("Trying to load classification data (df_classification)…")
try:
    df_classification = pd.read_csv(csv_file_cot, sep=';')
...
# LOAD coverage_per_test_cot.json
...
if os.path.exists(coverage_json):
    try:
        ...
        print("coverage_per_test_cot.json loaded. Keys:", len(coverage_dict))
    except Exception as err_covjson:
        print("Could not load coverage JSON:", err_covjson)
        coverage_dict = {}
else:
    print("coverage_per_test_cot.json not found. Continuing without coverage.")
...
if df_classification is not None:
    ...
    for idx, row in df_classification.iterrows():
        ...
        yaml_path = os.path.join(yaml_folder_cot, str(program_name) + "_report.yaml")
        # Default metrics
        ymetrics = {
            "Test Time (s)": np.nan,
...
try:
    ...
    print("appendix_metrics_table_cot.csv loaded successfully:", metrics_table_csv)
except Exception as errcsv:
    print("Error reading appendix_metrics_table_cot.csv:", metrics_table_csv)
    print(errcsv)
...
if "Detailed Classification" in merged_result_data.columns:
    ...
    plt.title("PLOT 2: Distribution of Classification Labels (4-way split, cot)")
    plt.tight_layout()
    plt.savefig(os.path.join(cot_plots_path, "plot2_pie_cot.png"))
    plt.close()
    print("Pie plot saved as plot2_pie_cot.png")
else:
...
if "Mutation Score" in merged_result_data.columns:
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(cot_plots_path, "plot3_boxplot_cot.png"))
    plt.close()
    print("Boxplot done (plot3_boxplot_cot.png)")
...
if "Mutation Score" in merged_result_data.columns:
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(cot_plots_path, "plot4_histogram_cot.png"))
    plt.close()
    print("Histogram done and saved as plot4_histogram_cot.png")
...
plt.title("PLOT 5: Distribution per Test Classification (Split, cot)")
plt.tight_layout()
plt.savefig(os.path.join(cot_plots_path, "plot5_enhanced_barplot_cot.png"))
plt.close()
print("PLOT 5 saved as plot5_enhanced_barplot_cot.png")
...
plt.ylabel("Mutation Score (%)")
plt.xlabel("Program")
plt.title("PLOT 6: Mutation Score per Program (Split/Annotated, cot)")
plt.tight_layout()
plt.savefig(os.path.join(cot_plots_path, "plot6_enhanced_mutscore_cot.png"))
plt.close()
print("PLOT 6 saved as plot6_enhanced_mutscore_cot.png")
...
if not outlier_rows.empty:
    ...
    plt.legend(handles=legend_elements)
plt.ylabel("Mutation Score (%)")
plt.title("PLOT 7: Outlier Mutation Scores with Failure Causes (cot)")
plt.tight_layout()
plt.savefig(os.path.join(cot_plots_path, "plot7_outlier_cot.png"))
plt.close()
print("PLOT 7 saved as plot7_outlier_cot.png")
...
if "Statement Coverage (%)" in merged_result_data.columns:
    ...
    if len(xvals.dropna()) > 2:
        ...
        plt.title("PLOT 8: Mutation Score vs Statement Coverage (cot)")
    plt.xlabel("Statement Coverage (%)")
    plt.ylabel("Mutation Score (%)")
    legend_elements = [mpatches.Patch(color=color_map[cat], label=cat) for cat in color_map]
    plt.legend(handles=legend_elements)
    plt.tight_layout()
    plt.savefig(os.path.join(cot_plots_path, "plot8_mutscore_vs_coverage_cot.png"))
    plt.close()
    print("PLOT 8 saved as plot8_mutscore_vs_coverage_cot.png")
...
if "Assertion Count" in merged_result_data.columns:
    ...
    plt.ylabel("Mutation Score (%)")
    plt.title("PLOT 9: Mutation Score vs Assertion Count (cot)")
    legend_elements = [mpatches.Patch(color=color_map[cat], label=cat) for cat in color_map]
    plt.legend(handles=legend_elements)
    plt.tight_layout()
    plt.savefig(os.path.join(cot_plots_path, "plot9_mutscore_vs_assertioncount_cot.png"))
    plt.close()
    print("PLOT 9 saved as plot9_mutscore_vs_assertioncount_cot.png")
...
if "Num Mutants" in merged_result_data.columns:
    ...
    plt.title("PLOT 10: Mutant Count by Test Class (cot)")
    plt.tight_layout()
    plt.savefig(os.path.join(cot_plots_path, "plot10_mutant_count_by_class_cot.png"))
    plt.close()
    print("PLOT 10 saved as plot10_mutant_count_by_class_cot.png")
...
if "Test Suite Size" in merged_result_data.columns:
    ...
    plt.ylabel("Mutation Score (%)")
    plt.title("PLOT 11: Mutation Score vs Test Suite Size (cot)")
    legend_elements = [mpatches.Patch(color=color_map[cat], label=cat) for cat in color_map]
    plt.legend(handles=legend_elements)
    plt.tight_layout()
    plt.savefig(os.path.join(cot_plots_path, "plot11_mutscore_vs_suitesize_cot.png"))
    plt.close()
    print("PLOT 11 saved as plot11_mutscore_vs_suitesize_cot.png")
...
try:
    ...
    plt.title("PLOT 12: Parallel Coordinates of Key Metrics by Class (cot)")
    plt.tight_layout()
    plt.savefig(os.path.join(cot_plots_path, "plot12_parallel_coordinates_cot.png"))
    plt.close()
    print("PLOT 12 saved as plot12_parallel_coordinates_cot.png")
except Exception as e:
...
if "Assertion Count" in merged_result_data.columns:
    ...
    plt.title("PLOT 13: Assertion Count by Test Class (cot)")
    plt.tight_layout()
    plt.savefig(os.path.join(cot_plots_path, "plot13_assertioncount_cot.png"))
    plt.close()
    print("PLOT 13 saved as plot13_assertioncount_cot.png")
...
if "Mutation Score" in merged_result_data.columns and "Detailed Classification" in merged_result_data.columns:
    try:
        ...
        plt.title("PLOT 14: Violin Plot of Mutation Score by Class (cot, matplotlib)")
        # Legend
        import matplotlib.patches as mpatches
        legend_elements = [mpatches.Patch(color=color_map[i], label=class_order[i]) for i in range(len(class_order))]
        plt.legend(handles=legend_elements, loc='best')
        plt.tight_layout()
        plt.savefig(os.path.join(cot_plots_path, "plot14_violinplot_cot.png"))
        plt.close()
        print("PLOT 14 saved as plot14_violinplot_cot.png")
...
if "Test Time (s)" in merged_result_data.columns:
    ...
    plt.title("PLOT 15: Total Time by Test Class (cot)")
    plt.tight_layout()
    plt.savefig(os.path.join(cot_plots_path, "plot15_totaltime_cot.png"))
    plt.close()
    print("PLOT 15 saved as plot15_totaltime_cot.png")

print("All plots (appendix and main text) saved in folder:", cot_plots_path)
"""