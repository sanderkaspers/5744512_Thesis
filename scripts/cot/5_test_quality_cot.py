...
# PATHS
cot_input_folder = "results/cot/filtered_mutation_results"
print("Set input folder for YAML files:", cot_input_folder)
csv_path_output = "results/cot/classification_summary_cot.csv"
...
print("Checking if input folder exists:", cot_input_folder)
if os.path.exists(cot_input_folder):
    folder_check_present = True
    print("Input folder present:", cot_input_folder)
else:
    print("Input folder not found:", cot_input_folder)
    print("ERROR: Input folder must exist or the script cannot continue.")
    assert os.path.exists(cot_input_folder)

# List files
print("Reading files from the input folder...")
for file_x in os.listdir(cot_input_folder):
...
for y_file in yaml_file_list:
    ...
    try:
        full_path = cot_input_folder + "/" + y_file
