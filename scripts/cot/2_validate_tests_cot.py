...
test_data_path = "datasets/GPT_4/Chain_of_thought/tests"
filtered_path = "datasets/GPT_4/Chain_of_thought/filtered_tests"
cot_result_dir = "results/cot"
LOG_FILE_FINAL_v2 = "results/cot/validation_log.txt"
JSON_NAME_X = "results/cot/filtered_tests_cot.json"
...
try:
    ...
    if not os.path.exists(cot_result_dir):
        print("Result folder (cot) does not exist, creating...")
        os.makedirs(cot_result_dir)
        print("Result folder (cot) created.")
    else:
        print("Result folder (cot) already exists.")
...
while idx_now <= total_tests_so_far:
    ...
    prog_path = "datasets/GPT_4/Chain_of_thought/programs/" + prog_name
    loader_unittesty = unittest.TestLoader()

    try:
        # Start coverage for the program code
        cov = coverage.Coverage(
            source=["datasets/GPT_4/Chain_of_thought/programs"]
        )
        cov.start()
...
while idx_now <= total_tests_so_far:
    ...
    cov = coverage.Coverage(source=["datasets/GPT_4/Chain_of_thought/programs"])
    try:
        cov.start()
...
cov_outpath = "results/cot/coverage_per_test_cot.json"
try:
...
manual_check_file = "results/cot/manual_checklist.txt"
try:
    ...
    for man_test in manual_checklist:
        ...
        program_path = os.path.join("datasets/GPT_4/Chain_of_thought/programs", program_file_name)
        test_file_name = man_test[0] + ".py"
        test_file_path = os.path.join("datasets/GPT_4/Chain_of_thought/tests", test_file_name)
