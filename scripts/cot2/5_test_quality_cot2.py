...
# PATHS
cot2_input_folder = "results/cot2/filtered_mutation_results"
print("Set input folder for YAML files:", cot2_input_folder)
csv_path_output = "results/cot2/classification_summary_cot2.csv"
...
print("Checking if input folder exists:", cot2_input_folder)
if os.path.exists(cot2_input_folder):
    folder_check_present = True
    print("Input folder present:", cot2_input_folder)
else:
    print("Input folder not found:", cot2_input_folder)
    print("ERROR: Input folder must exist or the script cannot continue.")
    assert os.path.exists(cot2_input_folder)

# List files
print("Reading files from the input folder...")
for file_x in os.listdir(cot2_input_folder):
...
for y_file in yaml_file_list:
    ...
    try:
        full_path = cot2_input_folder + "/" + y_file
