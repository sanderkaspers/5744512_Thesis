...
test_data_path = "datasets/GPT_4/Chain_of_thought2/tests"
filtered_path = "datasets/GPT_4/Chain_of_thought2/filtered_tests"
cot2_result_dir = "results/cot2"
LOG_FILE_FINAL_v2 = "results/cot2/validation_log.txt"
JSON_NAME_X = "results/cot2/filtered_tests_cot2.json"
...
try:
    ...
    if not os.path.exists(cot2_result_dir):
        print("Result folder (cot2) does not exist, creating...")
        os.makedirs(cot2_result_dir)
        print("Result folder (cot2) created.")
    else:
        print("Result folder (cot2) already exists.")
...
while idx_now <= total_tests_so_far:
    ...
    prog_path = "datasets/GPT_4/Chain_of_thought2/programs/" + prog_name
    loader_unittesty = unittest.TestLoader()

    try:
        # Start coverage for the program code
        cov = coverage.Coverage(
            source=["datasets/GPT_4/Chain_of_thought2/programs"]
        )
        cov.start()
...
while idx_now <= total_tests_so_far:
    ...
    cov = coverage.Coverage(source=["datasets/GPT_4/Chain_of_thought2/programs"])
    try:
        cov.start()
...
cov_outpath = "results/cot2/coverage_per_test_cot2.json"
try:
...
manual_check_file = "results/cot2/manual_checklist.txt"
try:
    ...
    for man_test in manual_checklist:
        ...
        program_path = os.path.join("datasets/GPT_4/Chain_of_thought2/programs", program_file_name)
        test_file_name = man_test[0] + ".py"
        test_file_path = os.path.join("datasets/GPT_4/Chain_of_thought2/tests", test_file_name)
