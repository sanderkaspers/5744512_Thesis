...
# PATHS
fsl_input_folder = "results/fsl/filtered_mutation_results"
print("Set input folder for YAML files:", fsl_input_folder)
csv_path_output = "results/fsl/classification_summary_fsl.csv"
...
print("Checking if input folder exists:", fsl_input_folder)
if os.path.exists(fsl_input_folder):
    folder_check_present = True
    print("Input folder present:", fsl_input_folder)
else:
    print("Input folder not found:", fsl_input_folder)
    print("ERROR: Input folder must exist or the script cannot continue.")
    assert os.path.exists(fsl_input_folder)

# List files
print("Reading files from the input folder...")
for file_x in os.listdir(fsl_input_folder):
...
for y_file in yaml_file_list:
    ...
    try:
        full_path = fsl_input_folder + "/" + y_file
