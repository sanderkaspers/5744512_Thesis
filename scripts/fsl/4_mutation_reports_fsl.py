...
# PATHS
out_dir_fsl = "results/fsl/filtered_mutation_results"
result_csv_path = os.path.join(out_dir_fsl, "mutation_summary_fsl.csv")
debug_log_file = os.path.join(out_dir_fsl, "report_debug_log.txt")
...
if not os.path.exists(out_dir_fsl):
    os.makedirs(out_dir_fsl, exist_ok=True)
    print("Output directory created:", out_dir_fsl)
else:
    print("Output directory already exists:", out_dir_fsl)
...
with open(debug_log_file, 'w', encoding='utf-8') as log_file:
    ...
    for random_report_file in os.listdir(out_dir_fsl):
        # Only look for files that match
        if random_report_file.startswith("program_") and random_report_file.endswith("_report.yaml"):
            list_report_files.append(os.path.join(out_dir_fsl, random_report_file))
