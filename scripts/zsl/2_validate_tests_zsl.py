...
test_data_path = "datasets/GPT_4/Zero_shot/tests"
filtered_path = "datasets/GPT_4/Zero_shot/filtered_tests"
zsl_result_dir = "results/zsl"
LOG_FILE_FINAL_v2 = "results/zsl/validation_log.txt"
JSON_NAME_X = "results/zsl/filtered_tests_zsl.json"
...
try:
    ...
    if not os.path.exists(zsl_result_dir):
        print("Result folder (zsl) does not exist, creating...")
        os.makedirs(zsl_result_dir)
        print("Result folder (zsl) created.")
    else:
        print("Result folder (zsl) already exists.")
...
while idx_now <= total_tests_so_far:
    ...
    prog_path = "datasets/GPT_4/Zero_shot/programs/" + prog_name
    loader_unittesty = unittest.TestLoader()

    try:
        # Start coverage for the program code
        cov = coverage.Coverage(
            source=["datasets/GPT_4/Zero_shot/programs"]
        )
        cov.start()
...
while idx_now <= total_tests_so_far:
    ...
    cov = coverage.Coverage(source=["datasets/GPT_4/Zero_shot/programs"])
    try:
        cov.start()
...
cov_outpath = "results/zsl/coverage_per_test_zsl.json"
try:
...
manual_check_file = "results/zsl/manual_checklist.txt"
try:
    ...
    for man_test in manual_checklist:
        ...
        program_path = os.path.join("datasets/GPT_4/Zero_shot/programs", program_file_name)
        test_file_name = man_test[0] + ".py"
        test_file_path = os.path.join("datasets/GPT_4/Zero_shot/tests", test_file_name)
