...
# PATHS
zsl_input_folder = "results/zsl/filtered_mutation_results"
print("Set input folder for YAML files:", zsl_input_folder)
csv_path_output = "results/zsl/classification_summary_zsl.csv"
...
print("Checking if input folder exists:", zsl_input_folder)
if os.path.exists(zsl_input_folder):
    folder_check_present = True
    print("Input folder present:", zsl_input_folder)
else:
    print("Input folder not found:", zsl_input_folder)
    print("ERROR: Input folder must exist or the script cannot continue.")
    assert os.path.exists(zsl_input_folder)

# List files
print("Reading files from the input folder...")
for file_x in os.listdir(zsl_input_folder):
...
for y_file in yaml_file_list:
    ...
    try:
        full_path = zsl_input_folder + "/" + y_file
