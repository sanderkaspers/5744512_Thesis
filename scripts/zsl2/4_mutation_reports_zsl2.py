...
# Register constructor for all python/module tags (zsl2 YAMLs!)
...
# PATHS
out_dir_zsl2 = "results/zsl2/filtered_mutation_results"
result_csv_path = os.path.join(out_dir_zsl2, "mutation_summary_zsl2.csv")
debug_log_file = os.path.join(out_dir_zsl2, "report_debug_log.txt")
...
if not os.path.exists(out_dir_zsl2):
    os.makedirs(out_dir_zsl2, exist_ok=True)
    print("Output directory created:", out_dir_zsl2)
else:
    print("Output directory already exists:", out_dir_zsl2)
...
with open(debug_log_file, 'w', encoding='utf-8') as log_file:
    ...
    for random_report_file in os.listdir(out_dir_zsl2):
        # Only look for files that match
        if random_report_file.startswith("program_") and random_report_file.endswith("_report.yaml"):
            list_report_files.append(os.path.join(out_dir_zsl2, random_report_file))
