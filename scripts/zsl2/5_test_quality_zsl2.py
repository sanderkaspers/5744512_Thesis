...
# PATHS
zsl2_input_folder = "results/zsl2/filtered_mutation_results"
print("Set input folder for YAML files:", zsl2_input_folder)
csv_path_output = "results/zsl2/classification_summary_zsl2.csv"
...
print("Checking if input folder exists:", zsl2_input_folder)
if os.path.exists(zsl2_input_folder):
    folder_check_present = True
    print("Input folder present:", zsl2_input_folder)
else:
    print("Input folder not found:", zsl2_input_folder)
    print("ERROR: Input folder must exist or the script cannot continue.")
    assert os.path.exists(zsl2_input_folder)

# List files
print("Reading files from the input folder...")
for file_x in os.listdir(zsl2_input_folder):
...
for y_file in yaml_file_list:
    ...
    try:
        full_path = zsl2_input_folder + "/" + y_file
