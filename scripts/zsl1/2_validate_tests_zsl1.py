...
test_data_path = "datasets/GPT_4/Zero_shot1/tests"
filtered_path = "datasets/GPT_4/Zero_shot1/filtered_tests"
zsl1_result_dir = "results/zsl1"
LOG_FILE_FINAL_v2 = "results/zsl1/validation_log.txt"
JSON_NAME_X = "results/zsl1/filtered_tests_zsl1.json"
...
try:
    ...
    if not os.path.exists(zsl1_result_dir):
        print("Result folder (zsl1) does not exist, creating...")
        os.makedirs(zsl1_result_dir)
        print("Result folder (zsl1) created.")
    else:
        print("Result folder (zsl1) already exists.")
...
while idx_now <= total_tests_so_far:
    ...
    prog_path = "datasets/GPT_4/Zero_shot1/programs/" + prog_name
    loader_unittesty = unittest.TestLoader()

    try:
        # Start coverage for the program code
        cov = coverage.Coverage(
            source=["datasets/GPT_4/Zero_shot1/programs"]
        )
        cov.start()
...
while idx_now <= total_tests_so_far:
    ...
    cov = coverage.Coverage(source=["datasets/GPT_4/Zero_shot1/programs"])
    try:
        cov.start()
...
cov_outpath = "results/zsl1/coverage_per_test_zsl1.json"
try:
...
manual_check_file = "results/zsl1/manual_checklist.txt"
try:
    ...
    for man_test in manual_checklist:
        ...
        program_path = os.path.join("datasets/GPT_4/Zero_shot1/programs", program_file_name)
        test_file_name = man_test[0] + ".py"
        test_file_path = os.path.join("datasets/GPT_4/Zero_shot1/tests", test_file_name)
