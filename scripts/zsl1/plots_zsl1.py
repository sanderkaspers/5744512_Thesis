"""
This script analyzes and visualizes mutation testing results from the 'zsl1' experiment.
It loads ALL data from one CSV ('appendix_metrics_table_zsl1F.csv') and generates all required appendix and main text visuals in one run.
"""
...
zsl1_plots_path = os.path.join(zsl1_result_path, 'plotss')
metrics_table_csv = os.path.join(zsl1_result_path, "appendix_metrics_table_zsl1.csv")
...
if "Detailed Classification" in merged_result_data.columns:
    ...
    plt.title("PLOT 1: Distribution of Classification Labels (4-way split, zsl1)")
    plt.tight_layout()
    plt.savefig(os.path.join(zsl1_plots_path, "plot1_pie_zsl1.png"))
    plt.close()
    print("Pie plot saved as plot1_pie_zsl1.png")
else:
    print("Detailed Classification column not found.")

# PLOT 2
print("PLOT 2: Boxplot of Mutation Scores (zsl1)")
if "Mutation Score" in merged_result_data.columns:
    ...
    plt.title(f"PLOT 2: Boxplot of Mutation Scores (zsl1)")
    plt.ylabel("Mutation Score (%)")
    plt.tight_layout()
    plt.savefig(os.path.join(zsl1_plots_path, "plot2_boxplot_zsl1.png"))
    plt.close()
    print("Boxplot done (plot2_boxplot_zsl1.png)")

# PLOT 3
print("PLOT 3: Histogram of Mutation Scores (zsl1)")
if "Mutation Score" in merged_result_data.columns:
    mut_hist_list = []
    ...
    plt.title(f"PLOT 3: Histogram of Mutation Scores (zsl1)")
    plt.xlabel("Mutation Score (%)")
    plt.ylabel("Number of Programs")
    plt.tight_layout()
    plt.savefig(os.path.join(zsl1_plots_path, "plot3_histogram_zsl1.png"))
    plt.close()
    print("Histogram done (plot3_histogram_zsl1.png)")
...
plt.title("PLOT 4: Distribution per Test Classification (Split, zsl1)")
plt.tight_layout()
plt.savefig(os.path.join(zsl1_plots_path, "plot4_enhanced_barplot_zsl1.png"))
plt.close()
print("PLOT 4 saved as plot4_enhanced_barplot_zsl1.png")
...
plt.ylabel("Mutation Score (%)")
plt.xlabel("Program")
plt.title("PLOT 5: Mutation Score per Program (Split/Annotated, zsl1)")
plt.tight_layout()
plt.savefig(os.path.join(zsl1_plots_path, "plot5_enhanced_mutscore_zsl1.png"))
plt.close()
print("PLOT 5 saved as plot5_enhanced_mutscore_zsl1.png")
...
if not outlier_rows.empty:
    ...
    plt.legend(handles=legend_elements)
plt.ylabel("Mutation Score (%)")
plt.title("PLOT 6: Outlier Mutation Scores with Failure Causes (zsl1)")
plt.tight_layout()
plt.savefig(os.path.join(zsl1_plots_path, "plot6_outlier_zsl1.png"))
plt.close()
print("PLOT 6 saved as plot6_outlier_zsl1.png")
...
if "Statement Coverage (%)" in merged_result_data.columns:
    ...
    if len(xvals.dropna()) > 2:
        ...
        plt.title("PLOT 7: Mutation Score vs Statement Coverage (zsl1)")
    plt.xlabel("Statement Coverage (%)")
    plt.ylabel("Mutation Score (%)")
    legend_elements = [mpatches.Patch(color=color_map[cat], label=cat) for cat in color_map]
    plt.legend(handles=legend_elements)
    plt.tight_layout()
    plt.savefig(os.path.join(zsl1_plots_path, "plot7_mutscore_vs_coverage_zsl1.png"))
    plt.close()
    print("PLOT 7 saved as plot7_mutscore_vs_coverage_zsl1.png")
...
if "Assertion Count" in merged_result_data.columns:
    ...
    plt.ylabel("Mutation Score (%)")
    plt.title("PLOT 8: Mutation Score vs Assertion Count (zsl1)")
    legend_elements = [mpatches.Patch(color=color_map[cat], label=cat) for cat in color_map]
    plt.legend(handles=legend_elements)
    plt.tight_layout()
    plt.savefig(os.path.join(zsl1_plots_path, "plot8_mutscore_vs_assertioncount_zsl1.png"))
    plt.close()
    print("PLOT 8 saved as plot8_mutscore_vs_assertioncount_zsl1.png")
...
if "Num Mutants" in merged_result_data.columns:
    ...
    plt.title("PLOT 9: Mutant Count by Test Class (zsl1)")
    plt.tight_layout()
    plt.savefig(os.path.join(zsl1_plots_path, "plot9_mutant_count_zsl1.png"))
    plt.close()
    print("PLOT 9 saved as plot9_mutant_count_zsl1.png")
...
if "Test Suite Size" in merged_result_data.columns:
    ...
    plt.ylabel("Mutation Score (%)")
    plt.title("PLOT 10: Mutation Score vs Test Suite Size (zsl1)")
    legend_elements = [mpatches.Patch(color=color_map[cat], label=cat) for cat in color_map]
    plt.legend(handles=legend_elements)
    plt.tight_layout()
    plt.savefig(os.path.join(zsl1_plots_path, "plot10_mutscore_vs_suitesize_zsl1.png"))
    plt.close()
    print("PLOT 10 saved as plot10_mutscore_vs_suitesize_zsl1.png")
...
try:
    ...
    plt.title("PLOT 11: Parallel Coordinates of Key Metrics by Class (zsl1)")
    plt.tight_layout()
    plt.savefig(os.path.join(zsl1_plots_path, "plot11_parallel_coordinates_zsl1.png"))
    plt.close()
    print("PLOT 11 saved as plot11_parallel_coordinates_zsl1.png")
...
if "Assertion Count" in merged_result_data.columns:
    ...
    plt.title("PLOT 12: Assertion Count by Test Class (zsl1)")
    plt.tight_layout()
    plt.savefig(os.path.join(zsl1_plots_path, "plot12_assertioncount_zsl1.png"))
    plt.close()
    print("PLOT 12 saved as plot12_assertioncount_zsl1.png")
...
if "Mutation Score" in merged_result_data.columns and "Detailed Classification" in merged_result_data.columns:
    try:
        ...
        plt.title("PLOT 13: Violin Plot of Mutation Score by Class (zsl1, matplotlib)")
        # Legend
        legend_elements = [mpatches.Patch(color=color_map[i], label=class_order[i]) for i in range(len(class_order))]
        plt.legend(handles=legend_elements, loc='best')
        plt.tight_layout()
        plt.savefig(os.path.join(zsl1_plots_path, "plot13_violinplot_zsl1.png"))
        plt.close()
        print("PLOT 13 saved as plot13_violinplot_zsl1.png")
...
if "Test Time (s)" in merged_result_data.columns:
    ...
    plt.title("PLOT 14: Total Time by Test Class (zsl1)")
    plt.tight_layout()
    plt.savefig(os.path.join(zsl1_plots_path, "plot14_totaltime_zsl1.png"))
    plt.close()
    print("PLOT 14 saved as plot14_totaltime_zsl1.png")

print("All 14 plots saved in folder:", zsl1_plots_path)
