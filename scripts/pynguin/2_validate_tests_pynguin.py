...
test_data_path = "datasets/pynguin/tests"
filtered_path = "datasets/pynguin/filtered_tests"
pynguin_result_dir = "results/pynguin"
LOG_FILE_FINAL_v2 = "results/pynguin/validation_log.txt"
JSON_NAME_X = "results/pynguin/filtered_tests_pynguin.json"
...
try:
    ...
    if not os.path.exists(pynguin_result_dir):
        print("Result folder (pynguin) does not exist, creating...")
        os.makedirs(pynguin_result_dir)
        print("Result folder (pynguin) created.")
    else:
        print("Result folder (pynguin) already exists.")
...
while idx_now <= total_tests_so_far:
    ...
    prog_path = "datasets/pynguin/programs/" + prog_name
    loader_unittesty = unittest.TestLoader()

    try:
        # Start coverage for the program code
        cov = coverage.Coverage(
            source=["datasets/pynguin/programs"]
        )
        cov.start()
    ...
    # Count number of test methods in suite
    num_test_methods = result_test_unit.testsRun  # <-- fix: reliable method count
    print("Number of test methods found in suite for", now_test_name, ":", num_test_methods)
    ...
    # Only copy test if no errors AND suite is not empty
    if errors_in_test_x == 0 and num_test_methods > 0:
        try:
    ...
    elif num_test_methods == 0:
        print("Test suite", now_test_name, "is empty, not copying to filtered_tests (excluded).")
        excluded_list.append(
            ("test_" + str(idx_now).zfill(3), "empty", 0, [], [])
        )
    else:
...
while idx_now <= total_tests_so_far:
    ...
    cov = coverage.Coverage(source=["datasets/pynguin/programs"])
    try:
        cov.start()
...
cov_outpath = "results/pynguin/coverage_per_test_pynguin.json"
try:
...
manual_check_file = "results/pynguin/manual_checklist.txt"
try:
    ...
    for man_test in manual_checklist:
        ...
        program_path = os.path.join("datasets/pynguin/programs", program_file_name)
        test_file_name = man_test[0] + ".py"
        test_file_path = os.path.join("datasets/pynguin/tests", test_file_name)
