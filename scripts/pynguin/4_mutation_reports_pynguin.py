...
def ignore_python_module_constructor(loader, node):
    # Just return a string or None for python/module tags
    return None

# Register constructor for all python/module tags (Pynguin YAMLs!)
IgnorePyModuleLoader.add_multi_constructor(
    'tag:yaml.org,2002:python/module:', lambda loader, suffix, node: None
)
# Fallback for all unknown tags
IgnorePyModuleLoader.add_multi_constructor('!', lambda loader, suffix, node: None)
# === END CUSTOM LOADER ===

# PATHS
out_dir_pynguin = "results/pynguin/filtered_mutation_results"
result_csv_path = os.path.join(out_dir_pynguin, "mutation_summary_pynguin.csv")
debug_log_file = os.path.join(out_dir_pynguin, "report_debug_log.txt")
...
if not os.path.exists(out_dir_pynguin):
    os.makedirs(out_dir_pynguin, exist_ok=True)
    print("Output directory created:", out_dir_pynguin)
else:
    print("Output directory already exists:", out_dir_pynguin)
...
def read_yaml_file(yaml_file_path):
    ...
    try:
        ...
        with open(yaml_file_path, "r", encoding="utf-8") as ff:
            result_dict = yaml.load(ff, Loader=IgnorePyModuleLoader)
        return result_dict
...
with open(debug_log_file, 'w', encoding='utf-8') as log_file:
    ...
    for random_report_file in os.listdir(out_dir_pynguin):
        # Only look for files that match
        if random_report_file.startswith("program_") and random_report_file.endswith("_report.yaml"):
            list_report_files.append(os.path.join(out_dir_pynguin, random_report_file))
