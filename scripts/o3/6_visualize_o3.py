"""
This script combines, analyzes, and visualizes mutation testing results from the 'o3' experiment.
...
# PATHS
o3_result_path = "results/o3"
yaml_folder_o3 = os.path.join(o3_result_path, 'filtered_mutation_results')
csv_file_o3 = os.path.join(o3_result_path, 'classification_summary_o3.csv')
o3_plots_path = os.path.join(o3_result_path, 'plots')
metrics_table_csv = os.path.join('results', 'o3', 'appendix_metrics_table_o3.csv')
coverage_json = os.path.join('results', 'o3', 'coverage_per_test_o3.json')
...
if not os.path.exists(o3_plots_path):
    try:
        os.makedirs(o3_plots_path)
        print("Directory created:", o3_plots_path)
    except Exception as plot_folder_error:
        print("Could not create folder:", o3_plots_path, "\nError:", plot_folder_error)
else:
    print("Directory already exists:", o3_plots_path)

# METRIC GATHERING BLOCK

# LOAD classification_summary_o3.csv as df_classification
print("Trying to load classification data (df_classification)…")
try:
    df_classification = pd.read_csv(csv_file_o3, sep=';')
...
# LOAD coverage_per_test_o3.json
...
if os.path.exists(coverage_json):
    try:
        ...
        print("coverage_per_test_o3.json loaded. Keys:", len(coverage_dict))
    except Exception as err_covjson:
        print("Could not load coverage JSON:", err_covjson)
        coverage_dict = {}
else:
    print("coverage_per_test_o3.json not found. Continuing without coverage.")
...
if df_classification is not None:
    ...
    for idx, row in df_classification.iterrows():
        ...
        yaml_path = os.path.join(yaml_folder_o3, str(program_name) + "_report.yaml")
        # Default metrics
        ymetrics = {
            "Test Time (s)": np.nan,
...
try:
    ...
    print("appendix_metrics_table_o3.csv loaded successfully:", metrics_table_csv)
except Exception as errcsv:
    print("Error reading appendix_metrics_table_o3.csv:", metrics_table_csv)
    print(errcsv)
...
if "Detailed Classification" in merged_result_data.columns:
    ...
    plt.title("PLOT 2: Distribution of Classification Labels (4-way split, o3)")
    plt.tight_layout()
    plt.savefig(os.path.join(o3_plots_path, "plot2_pie_o3.png"))
    plt.close()
    print("Pie plot saved as plot2_pie_o3.png")
else:
...
if "Mutation Score" in merged_result_data.columns:
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(o3_plots_path, "plot3_boxplot_o3.png"))
    plt.close()
    print("Boxplot done (plot3_boxplot_o3.png)")
...
if "Mutation Score" in merged_result_data.columns:
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(o3_plots_path, "plot4_histogram_o3.png"))
    plt.close()
    print("Histogram done and saved as plot4_histogram_o3.png")
...
plt.title("PLOT 5: Distribution per Test Classification (Split, o3)")
plt.tight_layout()
plt.savefig(os.path.join(o3_plots_path, "plot5_enhanced_barplot_o3.png"))
plt.close()
print("PLOT 5 saved as plot5_enhanced_barplot_o3.png")
...
plt.ylabel("Mutation Score (%)")
plt.xlabel("Program")
plt.title("PLOT 6: Mutation Score per Program (Split/Annotated, o3)")
plt.tight_layout()
plt.savefig(os.path.join(o3_plots_path, "plot6_enhanced_mutscore_o3.png"))
plt.close()
print("PLOT 6 saved as plot6_enhanced_mutscore_o3.png")
...
if not outlier_rows.empty:
    ...
    plt.legend(handles=legend_elements)
plt.ylabel("Mutation Score (%)")
plt.title("PLOT 7: Outlier Mutation Scores with Failure Causes (o3)")
plt.tight_layout()
plt.savefig(os.path.join(o3_plots_path, "plot7_outlier_o3.png"))
plt.close()
print("PLOT 7 saved as plot7_outlier_o3.png")
...
if "Statement Coverage (%)" in merged_result_data.columns:
    ...
    if len(xvals.dropna()) > 2:
        ...
        plt.title("PLOT 8: Mutation Score vs Statement Coverage (o3)")
    plt.xlabel("Statement Coverage (%)")
    plt.ylabel("Mutation Score (%)")
    legend_elements = [mpatches.Patch(color=color_map[cat], label=cat) for cat in color_map]
    plt.legend(handles=legend_elements)
    plt.tight_layout()
    plt.savefig(os.path.join(o3_plots_path, "plot8_mutscore_vs_coverage_o3.png"))
    plt.close()
    print("PLOT 8 saved as plot8_mutscore_vs_coverage_o3.png")
...
if "Assertion Count" in merged_result_data.columns:
    ...
    plt.ylabel("Mutation Score (%)")
    plt.title("PLOT 9: Mutation Score vs Assertion Count (o3)")
    legend_elements = [mpatches.Patch(color=color_map[cat], label=cat) for cat in color_map]
    plt.legend(handles=legend_elements)
    plt.tight_layout()
    plt.savefig(os.path.join(o3_plots_path, "plot9_mutscore_vs_assertioncount_o3.png"))
    plt.close()
    print("PLOT 9 saved as plot9_mutscore_vs_assertioncount_o3.png")
...
if "Num Mutants" in merged_result_data.columns:
    ...
    plt.title("PLOT 10: Mutant Count by Test Class (o3)")
    plt.tight_layout()
    plt.savefig(os.path.join(o3_plots_path, "plot10_mutant_count_o3.png"))
    plt.close()
    print("PLOT 10 saved as plot10_mutant_count_o3.png")
...
if "Test Suite Size" in merged_result_data.columns:
    ...
    plt.ylabel("Mutation Score (%)")
    plt.title("PLOT 11: Mutation Score vs Test Suite Size (o3)")
    legend_elements = [mpatches.Patch(color=color_map[cat], label=cat) for cat in color_map]
    plt.legend(handles=legend_elements)
    plt.tight_layout()
    plt.savefig(os.path.join(o3_plots_path, "plot11_mutscore_vs_suitesize_o3.png"))
    plt.close()
    print("PLOT 11 saved as plot11_mutscore_vs_suitesize_o3.png")
...
try:
    ...
    plt.title("PLOT 12: Parallel Coordinates of Key Metrics by Class (o3)")
    plt.tight_layout()
    plt.savefig(os.path.join(o3_plots_path, "plot12_parallel_coordinates_o3.png"))
    plt.close()
    print("PLOT 12 saved as plot12_parallel_coordinates_o3.png")
except Exception as e:
...
if "Assertion Count" in merged_result_data.columns:
    ...
    plt.title("PLOT 13: Assertion Count by Test Class (o3)")
    plt.tight_layout()
    plt.savefig(os.path.join(o3_plots_path, "plot13_assertioncount_o3.png"))
    plt.close()
    print("PLOT 13 saved as plot13_assertioncount_o3.png")
...
if "Mutation Score" in merged_result_data.columns and "Detailed Classification" in merged_result_data.columns:
    try:
        ...
        plt.title("PLOT 14: Violin Plot of Mutation Score by Class (o3, matplotlib)")
        # Legend
        import matplotlib.patches as mpatches
        legend_elements = [mpatches.Patch(color=color_map[i], label=class_order[i]) for i in range(len(class_order))]
        plt.legend(handles=legend_elements, loc='best')
        plt.tight_layout()
        plt.savefig(os.path.join(o3_plots_path, "plot14_violinplot_o3.png"))
        plt.close()
        print("PLOT 14 saved as plot14_violinplot_o3.png")
...
if "Test Time (s)" in merged_result_data.columns:
    ...
    plt.title("PLOT 15: Total Time by Test Class (o3)")
    plt.tight_layout()
    plt.savefig(os.path.join(o3_plots_path, "plot15_totaltime_o3.png"))
    plt.close()
    print("PLOT 15 saved as plot15_totaltime_o3.png")

print("All plots (appendix and main text) saved in folder:", o3_plots_path)
"""