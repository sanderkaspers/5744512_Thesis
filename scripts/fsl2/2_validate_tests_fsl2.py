...
test_data_path = "datasets/GPT_4/Few_shot2/tests"
filtered_path = "datasets/GPT_4/Few_shot2/filtered_tests"
fsl2_result_dir = "results/fsl2"
LOG_FILE_FINAL_v2 = "results/fsl2/validation_log.txt"
JSON_NAME_X = "results/fsl2/filtered_tests_fsl2.json"
...
try:
    ...
    if not os.path.exists(fsl2_result_dir):
        print("Result folder (fsl2) does not exist, creating...")
        os.makedirs(fsl2_result_dir)
        print("Result folder (fsl2) created.")
    else:
        print("Result folder (fsl2) already exists.")
...
while idx_now <= total_tests_so_far:
    ...
    prog_path = "datasets/GPT_4/Few_shot2/programs/" + prog_name
    loader_unittesty = unittest.TestLoader()

    try:
        # Start coverage for the program code
        cov = coverage.Coverage(
            source=["datasets/GPT_4/Few_shot2/programs"]
        )
        cov.start()
...
while idx_now <= total_tests_so_far:
    ...
    cov = coverage.Coverage(source=["datasets/GPT_4/Few_shot2/programs"])
    try:
        cov.start()
...
cov_outpath = "results/fsl2/coverage_per_test_fsl2.json"
try:
...
manual_check_file = "results/fsl2/manual_checklist.txt"
try:
    ...
    for man_test in manual_checklist:
        ...
        program_path = os.path.join("datasets/GPT_4/Few_shot2/programs", program_file_name)
        test_file_name = man_test[0] + ".py"
        test_file_path = os.path.join("datasets/GPT_4/Few_shot2/tests", test_file_name)
