...
# PATHS
fsl2_input_folder = "results/fsl2/filtered_mutation_results"
print("Set input folder for YAML files:", fsl2_input_folder)
csv_path_output = "results/fsl2/classification_summary_fsl2.csv"
...
print("Checking if input folder exists:", fsl2_input_folder)
if os.path.exists(fsl2_input_folder):
    folder_check_present = True
    print("Input folder present:", fsl2_input_folder)
else:
    print("Input folder not found:", fsl2_input_folder)
    print("ERROR: Input folder must exist or the script cannot continue.")
    assert os.path.exists(fsl2_input_folder)

# List files
print("Reading files from the input folder...")
for file_x in os.listdir(fsl2_input_folder):
...
for y_file in yaml_file_list:
    ...
    try:
        full_path = fsl2_input_folder + "/" + y_file
