"""
This script analyzes and visualizes mutation testing results from the 'fsl2' experiment.
It loads ALL data from one CSV ('appendix_metrics_table_fsl2F.csv') and generates all required appendix and main text visuals in one run.
"""
...
fsl2_plots_path = os.path.join(fsl2_result_path, 'plotss')
metrics_table_csv = os.path.join(fsl2_result_path, "appendix_metrics_table_fsl2.csv")
...
if "Detailed Classification" in merged_result_data.columns:
    ...
    plt.title("PLOT 1: Distribution of Classification Labels (4-way split, fsl2)")
    plt.tight_layout()
    plt.savefig(os.path.join(fsl2_plots_path, "plot1_pie_fsl2.png"))
    plt.close()
    print("Pie plot saved as plot1_pie_fsl2.png")
else:
    print("Detailed Classification column not found.")

# PLOT 2
print("PLOT 2: Boxplot of Mutation Scores (fsl2)")
if "Mutation Score" in merged_result_data.columns:
    ...
    plt.title(f"PLOT 2: Boxplot of Mutation Scores (fsl2)")
    plt.ylabel("Mutation Score (%)")
    plt.tight_layout()
    plt.savefig(os.path.join(fsl2_plots_path, "plot2_boxplot_fsl2.png"))
    plt.close()
    print("Boxplot done (plot2_boxplot_fsl2.png)")

# LOT 3
print("PLOT 3: Histogram of Mutation Scores (fsl2)")
if "Mutation Score" in merged_result_data.columns:
    mut_hist_list = []
    ...
    plt.title(f"PLOT 3: Histogram of Mutation Scores (fsl2)")
    plt.xlabel("Mutation Score (%)")
    plt.ylabel("Number of Programs")
    plt.tight_layout()
    plt.savefig(os.path.join(fsl2_plots_path, "plot3_histogram_fsl2.png"))
    plt.close()
    print("Histogram done (plot3_histogram_fsl2.png)")
...
plt.title("PLOT 4: Distribution per Test Classification (Split, fsl2)")
plt.tight_layout()
plt.savefig(os.path.join(fsl2_plots_path, "plot4_enhanced_barplot_fsl2.png"))
plt.close()
print("PLOT 4 saved as plot4_enhanced_barplot_fsl2.png")
...
plt.ylabel("Mutation Score (%)")
plt.xlabel("Program")
plt.title("PLOT 5: Mutation Score per Program (Split/Annotated, fsl2)")
plt.tight_layout()
plt.savefig(os.path.join(fsl2_plots_path, "plot5_enhanced_mutscore_fsl2.png"))
plt.close()
print("PLOT 5 saved as plot5_enhanced_mutscore_fsl2.png")
...
if not outlier_rows.empty:
    ...
    plt.legend(handles=legend_elements)
plt.ylabel("Mutation Score (%)")
plt.title("PLOT 6: Outlier Mutation Scores with Failure Causes (fsl2)")
plt.tight_layout()
plt.savefig(os.path.join(fsl2_plots_path, "plot6_outlier_fsl2.png"))
plt.close()
print("PLOT 6 saved as plot6_outlier_fsl2.png")
...
if "Statement Coverage (%)" in merged_result_data.columns:
    ...
    if len(xvals.dropna()) > 2:
        ...
        plt.title("PLOT 7: Mutation Score vs Statement Coverage (fsl2)")
    plt.xlabel("Statement Coverage (%)")
    plt.ylabel("Mutation Score (%)")
    legend_elements = [mpatches.Patch(color=color_map[cat], label=cat) for cat in color_map]
    plt.legend(handles=legend_elements)
    plt.tight_layout()
    plt.savefig(os.path.join(fsl2_plots_path, "plot7_mutscore_vs_coverage_fsl2.png"))
    plt.close()
    print("PLOT 7 saved as plot7_mutscore_vs_coverage_fsl2.png")
...
if "Assertion Count" in merged_result_data.columns:
    ...
    plt.ylabel("Mutation Score (%)")
    plt.title("PLOT 8: Mutation Score vs Assertion Count (fsl2)")
    legend_elements = [mpatches.Patch(color=color_map[cat], label=cat) for cat in color_map]
    plt.legend(handles=legend_elements)
    plt.tight_layout()
    plt.savefig(os.path.join(fsl2_plots_path, "plot8_mutscore_vs_assertioncount_fsl2.png"))
    plt.close()
    print("PLOT 8 saved as plot8_mutscore_vs_assertioncount_fsl2.png")
...
if "Num Mutants" in merged_result_data.columns:
    ...
    plt.title("PLOT 9: Mutant Count by Test Class (fsl2)")
    plt.tight_layout()
    plt.savefig(os.path.join(fsl2_plots_path, "plot9_mutant_fsl2.png"))
    plt.close()
    print("PLOT 9 saved as plot9_mutant_count_fsl2.png")
...
if "Test Suite Size" in merged_result_data.columns:
    ...
    plt.ylabel("Mutation Score (%)")
    plt.title("PLOT 10: Mutation Score vs Test Suite Size (fsl2)")
    legend_elements = [mpatches.Patch(color=color_map[cat], label=cat) for cat in color_map]
    plt.legend(handles=legend_elements)
    plt.tight_layout()
    plt.savefig(os.path.join(fsl2_plots_path, "plot10_mutscore_vs_suitesize_fsl2.png"))
    plt.close()
    print("PLOT 10 saved as plot10_mutscore_vs_suitesize_fsl2.png")
...
try:
    ...
    plt.title("PLOT 11: Parallel Coordinates of Key Metrics by Class (fsl2)")
    plt.tight_layout()
    plt.savefig(os.path.join(fsl2_plots_path, "plot11_parallel_coordinates_fsl2.png"))
    plt.close()
    print("PLOT 11 saved as plot11_parallel_coordinates_fsl2.png")
...
if "Assertion Count" in merged_result_data.columns:
    ...
    plt.title("PLOT 12: Assertion Count by Test Class (fsl2)")
    plt.tight_layout()
    plt.savefig(os.path.join(fsl2_plots_path, "plot12_assertioncount_fsl2.png"))
    plt.close()
    print("PLOT 12 saved as plot12_assertioncount_fsl2.png")
...
if "Mutation Score" in merged_result_data.columns and "Detailed Classification" in merged_result_data.columns:
    try:
        ...
        plt.title("PLOT 13: Violin Plot of Mutation Score by Class (fsl2, matplotlib)")
        # Legend
        legend_elements = [mpatches.Patch(color=color_map[i], label=class_order[i]) for i in range(len(class_order))]
        plt.legend(handles=legend_elements, loc='best')
        plt.tight_layout()
        plt.savefig(os.path.join(fsl2_plots_path, "plot13_violinplot_fsl2.png"))
        plt.close()
        print("PLOT 13 saved as plot13_violinplot_fsl2.png")
...
if "Test Time (s)" in merged_result_data.columns:
    ...
    plt.title("PLOT 14: Total Time by Test Class (fsl2)")
    plt.tight_layout()
    plt.savefig(os.path.join(fsl2_plots_path, "plot14_totaltime_fsl2.png"))
    plt.close()
    print("PLOT 14 saved as plot14_totaltime_fsl2.png")

print("All 14 plots saved in folder:", fsl2_plots_path)
