...
# Je doelmap:
test_dir = Path(r"C:\Users\skaspers\OneDrive - Capgemini\Documents\Prompted Testsuite Analysis\datasets\GPT_4\Few_shot2\tests")
...
def fix_file(file_path):
    ...
    for i, line in enumerate(lines):
        if line.startswith('from datasets.GPT_4.Few_shot2.programs'):
            # Pak progjeXXX nummer uit de foute import
            match = re.search(r'program_(\d+)', line)
            progje_nummer = match.group(1) if match else '001'
            new_line = f'from datasets.GPT_4.Few_shot2.programs.program_{progje_nummer} import *\n'
